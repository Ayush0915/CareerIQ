...
def fetch_jsearch_jobs_sync(keywords: List[str], location: str = "India") -> List[Dict]:
    ...
    try:
        logger.info(f"Calling JSearch: {primary_query}")
        ...
        if response.status_code != 200 or not data.get("data", []):
            logger.info(f"Primary yield 0, trying fallback: {fallback_query}")
            response = requests.get(
                "https://jsearch.p.rapidapi.com/search",
                params={
        ...
        if response.status_code == 200:
            logger.info(f"JSearch data count: {len(data.get('data', []))}")
        ...
        logger.error(f"JSearch error: {type(e).__name__}: {e}")
